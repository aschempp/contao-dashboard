...
(
	...
	(
		...
		'default'                     => '{text_legend},headline,text,addImage;{restrict_legend},restrictUsers,restrictGroups;{publish_legend},published,start,stop;{expert_legend:hide},style,bgcolor,cssID'
	),
...
class tl_dashboard extends Backend
{
	...
	public function pasteItem(DataContainer $dc, $row, $table, $cr, $arrClipboard=false)
	{
		...
		$objPage = $this->Database->prepare("SELECT * FROM tl_dashboard WHERE id=?")
								  ->limit(1)
								  ->execute($row['pid']);
